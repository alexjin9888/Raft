...
public class RaftClient {
    ...
    /**
     * The current request for which we have not yet received a successful 
     * reply
     */
    ...
    /**
     * Processes a received serializable object and conditionally sends 
     * another request depending on the contents of the object.
    ...
    public synchronized void handleSerializable(Serializable object) {
        ...
        if (outstandingRequest == null || 
                outstandingRequest.commandId != reply.commandId) {
    ...
    private synchronized void sendRetryingRequest() {
        if (leaderAddress == null) {
            leaderAddress = serverAddresses.get(ThreadLocalRandom.current()
                    .nextInt(serverAddresses.size()));
        }
        ...
        retryRequestTask = new CheckingCancelTimerTask() {
            public void run() {
                synchronized(RaftClient.this) {
                    ...
                    leaderAddress = serverAddresses.get(ThreadLocalRandom
                            .current().nextInt(serverAddresses.size()));
                    assert(outstandingRequest != null);
                    networkManager.sendSerializable(leaderAddress, 
                            outstandingRequest);
                }
            }
        };

        retryRequestTimer.scheduleAtFixedRate(retryRequestTask, 
    ...
    private synchronized void waitForAndProcessInput() {
        ...
        numCommandsRead += 1;
        outstandingRequest = new ClientRequest(numCommandsRead, myAddress, 
                command);
        sendRetryingRequest();
    }
